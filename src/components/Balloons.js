        ...
        >
          <g className="links">
            {dataset.map((datum, i) => {
              ...
              const xoffs = c.CHART_AREA_WIDTH / 4
              const x1 = MOUNTAIN_X_SCALE(datum[xMountainDimension]) + xoffs
              const y1 =
                c.CHART_AREA_HEIGHT -
                c.MARGIN.bottom -
                MOUNTAIN_Y_SCALE(genresPopularity[genres.indexOf(datum.main_genre)])
          ...
          <g
            className="lower"
            transform={`translate(${c.CHART_AREA_WIDTH / 4}, ${c.CHART_AREA_HEIGHT - c.MARGIN.bottom})`}
          >
            {genres.map((genre, i) => {
